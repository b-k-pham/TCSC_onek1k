...
for (i in 1:ntissues){
  ...
  tissueassign_labels <- c(tissueassign_labels,res$tissueassign_labels)
...
if(length(remove_genes)>0){
  ...
  chrs <- chrs[-remove_genes]
  tissueassign <- tissueassign[-remove_genes]
  tissueassign_labels <- tissueassign_labels[-remove_genes]
  sources <- sources[-remove_genes]
  X <- X[-remove_genes,]
}
...
for (idx in 1:nrow(trait_h2g_N)){
  ...
  tryCatch({
    debug_TCSC(trait, h2g, N, coreg_path)
  }, error = function(e) {
    skip_to_next <<- TRUE  # Mark to skip this iteration if an error occurs
  })
...
for (i in unlist(bad_sumstats)){
  print(paste0(i,' ',trait_h2g_N[[i,1]]))
}
...
trait <- trait_h2g_N[[40,1]]
h2g <- as.numeric(trait_h2g_N[[40,2]])
N <- as.numeric(trait_h2g_N[[40,3]])
coreg_path <- "TCSC/analysis/InputCoreg_onek1k_gtex_TCSC.RData"
run_TCSC_by_idx(trait,h2g,N,coreg_path)


```

```{r}
bad_sumstats = unlist(bad_sumstats)
good_sstats_id = trait_h2g_N[-bad_sumstats,]$Trait_Identifier
good_nSNPs = list()
...
ggsave("TCSC_heatmap_QC_immune_wgtex_full_normal.pdf",plot = p_tot, width = 40, height = 20, unit = 'cm')
